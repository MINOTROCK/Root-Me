# Root-Me

This repository contains all my write-ups for the Root Me CTF challenges I have completed.

The goal is to document my progress on the Root Me platform.

I want to reach 1000 points before September 2026.

Before reading the list, please note that a CSV and an Excel version is available, `LIST.csv` or `LIST.xlsx`.

The Excel verion is enhance compared to the CSV, which is provided as a standard comma-separated values,

may not be user-friendly with some European Excel configurations.

## Stats

🏆 Total Points: 20

✅ Challenges solved: 2

<details>
🔎 Forensic: 2
</details>

## List

### [ Deleted file](https://github.com/MINOTROCK/Root-Me/blob/main/001_deleted_file_(df).md)🔗

🔎 **Category:** Forensic 🏆 **Points:** 5 🟢 **Level:** Very easy 👤 **Author:** Manah 🗓️ **Date:** 24/05/2023 ✅ **Validating:** 05/04/2026  

<details>
Your cousin found a USB drive in the library this morning. He’s not very good with computers, so he’s hoping you can find the owner of this stick! The flag is the owner’s identity in the form "firstname_lastname".
</details>

---

### [Capture this](https://github.com/MINOTROCK/Root-Me/blob/main/002_capture_this_(ct).md)🔗

🔎 **Category:** Forensic 🏆 **Points:** 15 🟡 **Level:** easy 👤 **Author:** Zey_Roxx 🗓️ **Date:** 20/10/2023 ✅ **Validating:** 08/04/2026  

<details>
An employee has lost his Keepass password. He couldn’t remember it, and couldn’t find his password file. After hours of searching, it turns out that he has sent a screen of his passwords to one of his colleagues, but it’s still nowhere to be found. He’s asking for your help to find him. It's up to you.
</details>

---
